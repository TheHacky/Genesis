...
namespace Genesis.CLI
{
	...
	{
		private static readonly ILogger LOGGER = Log.ForContext(typeof(CodeGeneratorTools));
		...
		public static CodeGeneratorRunner CodeGeneratorFromPreferences(IGenesisConfig settings)
		{
			...
			var enabledInstancesOf1 = GetEnabledInstancesOf<IPreProcessor>(instances, andConfigure.EnabledPreProcessors);
			var enabledInstancesOf2 = GetEnabledInstancesOf<IDataProvider>(instances, andConfigure.EnabledDataProviders);
			var enabledInstancesOf3 = GetEnabledInstancesOf<ICodeGenerator>(instances, andConfigure.EnabledCodeGenerators);
			var enabledInstancesOf4 = GetEnabledInstancesOf<IPostProcessor>(instances, andConfigure.EnabledPostProcessors);
