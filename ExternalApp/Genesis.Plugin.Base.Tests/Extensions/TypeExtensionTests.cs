...
namespace Genesis.Plugin.Base.Tests
{
	[TestFixture]
	internal sealed class TypeExtensionTests
	{
		...
		{
			Assert.AreEqual(
				"Genesis.Plugin.Base.Tests.TypeExtensionTests.GoodFoo[]",
				typeof(GoodFoo[]).GetFullTypeName());

			Assert.AreEqual(
				"System.Collections.Generic.List<Genesis.Plugin.Base.Tests.TypeExtensionTests.GoodFoo>",
				typeof(List<GoodFoo>).GetFullTypeName());

			Assert.AreEqual(
				"System.Collections.Generic.Dictionary<System.Int32,Genesis.Plugin.Base.Tests.TypeExtensionTests.GoodFoo>",
